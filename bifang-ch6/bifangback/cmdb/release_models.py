...
class ReleaseStatus(BaseModel):
    # ['Create', 'Building', 'BuildFailed', \
    #       'Build', 'Ready', 'Ongoing', 'Success', 'Failed']
    # ['创建', '编译中', '编译失败', \
    #       '编译成功', '准备部署', '部署中', '部署成功', '部署失败']
    status_value = models.CharField(max_length=1024,
                                    blank=True,
                                    verbose_name="状态值")
